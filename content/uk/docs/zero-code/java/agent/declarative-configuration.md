...
> [!WARNING]
>
> Декларативна конфігурація є стабільною. Частини, які все ще експериментальні, мають суфікс `/development`. Підтримка декларативної конфігурації Java-агентом все ще експериментальна.

## Підтримувані версії {#supported-versions}

Декларативна конфігурація підтримується в **OpenTelemetry Java agent версії 2.26.0 і пізніших**.
...
Ця сторінка присвячена особливостям роботи [Java-агента OpenTelemetry](https://github.com/open-telemetry/opentelemetry-java-instrumentation). Для Spring Boot starter дивіться
[Декларативна конфігурація Spring Boot starter](/docs/zero-code/java/spring-boot-starter/declarative-configuration/).

## Перетворення наявної конфігурації {#convert-your-existing-configuration}

{{< uk/dc-converter source="agent" >}}
...
Параметри, що стосуються агента (імена починаються з `otel.javaagent.`), розміщуються в розділі `distribution`:

```yaml
distribution:
  javaagent:
    instrumentation:
      default_enabled: false # було otel.instrumentation.common.default-enabled
      enabled:
        - tomcat
        - spring_webmvc
      disabled:
        - armeria_grpc
    exclude_classes: # було otel.javaagent.exclude-classes
      - com.example.excluded.Class1
    exclude_class_loaders: # було otel.javaagent.exclude-class-loaders
