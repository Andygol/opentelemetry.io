...
# prettier-ignore
cSpell:ignore: kube kubecache kubeconfig replicaset statefulset Зменшіть висококардинальну
---
...
## Вибір трейсу {#trace-selection}

Для експортованих OpenTelemetry трейсів використовуйте ключ `traces` (не назву метрики) в `attributes.select`. Він контролює додаткове декорування трейсу, таке як `db.query.text`, `url.query`, атрибути GenAI payload та `db.response.error`.
...
`db.response.error` не є частиною семантичних домовленостей OpenTelemetry. OBI використовує цей рядок лише як конфігураційний прапорець у `attributes.select.traces`.

| Умова                    | Поведінка                                                                                                                                                                                                                                                                                                 |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Не включено (стандартно) | У випадку невдалих відрізків, повʼязаних з базою даних (SQL, Redis, MongoDB, Couchbase, Memcached, SQL++ через HTTP), `span.status.message` залишається порожнім. Це узгоджується з тим, як поводяться інші необовʼязкові атрибути (наприклад, `db.query.text`) — коли вони не вибрані, вони опускаються. |
| Включено                 | У тих же відрізках, `span.status.message` встановлюється на фактичний опис помилки, отриманий з протоколу відповіді.                                                                                                                                                                                      |

`db.response.error` ніколи не додається як атрибут відрізка в трейсах OTLP. Під час експорту OBI використовує це значення лише для формування `span.status.message` для відрізків бази даних, а потім видаляє атрибут з експортованого відрізка. Увімкнення цієї опції змінює опис статусу, а не окреме поле `db.response.error` у відрізку.

Ця опція існує, оскільки рядки помилок можуть містити конфіденційну або висококардинальну інформацію (назви схем, фрагменти запитів або значення даних).
...
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ---------- |
| `context_propagation`<br>`OTEL_EBPF_BPF_CONTEXT_PROPAGATION`     | Керує методом передачі контексту трасування. Прийняті значення: `all`, `headers`, `tcp`, `headers,tcp`, `disabled`. Для отримання додаткової інформації зверніться до [розділу про передачу контексту](#context-propagation). | string  | disabled   |
...
Ви можете вимкнути поширення на рівні TCP та програми управління трафіком Linux, встановивши параметр `context_propagation="headers"`. Цей режим повністю сумісний з будь-якою бібліотекою розподіленого трасування OpenTelemetry

Значення поширення контексту:

- `all`: Увімкнути як заголовка HTTP, так і TCP для поширення контексту
- `headers`: Увімкнути поширення контексту лише через HTTP-заголовки
- `tcp`: Увімкнути поширення контексту лише через TCP-пакети
- `headers,tcp`: Увімкнути обидва методи явно
- `disabled`: Вимкнути поширення контексту трасування

`http` приймається як псевдонім для `headers`, але `headers` є рекомендованою назвою в прикладах та конфігурації.
...
gRPC та HTTP/2 не підтримуються для цього мережевого рівня.
...
Опція `heuristic sql detect` дозволяє OBI виявляти запити SQL-клієнтів, перевіряючи оператори запитів, навіть якщо протокол безпосередньо не підтримується. Стандартно OBI виявляє запити SQL-клієнтів за їх форматом бінарного протоколу. Якщо ви використовуєте технологію бази даних, яка безпосередньо не підтримується OBI, ви можете увімкнути цю опцію, щоб отримати телеметрію клієнта бази даних. Ця опція типово не увімкнена, оскільки вона може створювати хибнопозитивні результати, наприклад, якщо застосунок надсилає текст SQL для ведення журналу через TCP-зʼєднання. В даний час OBI нативно підтримує бінарні протоколи PostgreSQL, MySQL та MSSQL.

### Розширення HTTP header та body для відрізків{#http-header-enrichment-for-spans}

OBI може прикріплювати вибрані HTTP-заголовки та вибрані поля тіла HTTP до елементів відрізків через секцію конфігурації `ebpf.payload_extraction.http.enrichment`. Це корисно, коли ви хочете передавати заголовки бізнес-логіки або заголовки маршрутизації у трейсах без ручного інструментування застосунку.

Механізм розширення працює на основі правил:

- Встановіть `enabled: true`, щоб увімкнути розширення заголовків HTTP та полів тіла.
- Використовуйте `policy.default_action.headers` та `policy.default_action.body` у YAML, щоб визначити, чи слід включати заголовки або вміст тіла, що не відповідають критеріям. Стандартним значенням для обох є `exclude`.
- Використовуйте правила `obfuscate`, щоб замаскувати конфіденційні значення заголовків або полів JSON перед їх прикріпленням до відрізків.
- Правила оцінюються в порядку їх визначення.
...
Наступні змінні середовища керують глобальною поведінкою розширення:

- `OTEL_EBPF_HTTP_ENRICHMENT_ENABLED`
- `OTEL_EBPF_HTTP_ENRICHMENT_OBFUSCATION_STRING`

Параметри `policy.default_action.headers` та `policy.default_action.body` налаштовуються лише в YAML; для цих значень немає змінних середовища.

Правила самі налаштовуються в YAML. Правила заголовків використовують `match.patterns` та необовʼязковий `case_sensitive`. Правила тіла використовують `match.url_path_patterns`, `match.methods` та `match.obfuscation_json_paths`.

Витягування тіла вимагає захоплення HTTP payload. Збільште `ebpf.buffer_sizes.http`, щоб OBI міг захоплювати байти запиту або відповіді, які ви хочете збагачувати. Ліміт застосовується незалежно до запитів і відповідей.
...
| YAML<br>змінна середовища                                                   | Опис                                                                                                                                                                                                                  | Тип            | Стандартно     |
| --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- |
| `enable`<br>`OTEL_EBPF_KUBE_METADATA_ENABLE`                                | Вмикає чи вимикає декорацію метаданих Kubernetes. Встановіть `autodetect`, щоб увімкнути, якщо ви працюєте в Kubernetes. Для отримання додаткової інформації див. розділ [увімкнення Kubernetes](#enable-kubernetes). | boolean/string | false          |
...
| `reconnect_initial_interval`<br>`OTEL_EBPF_KUBE_RECONNECT_INITIAL_INTERVAL` | Початкова затримка перед повторним підключенням до API Kubernetes після втрати зʼєднання. Для отримання додаткової інформації див. розділ [початкова затримка повторного підключення](#reconnect-initial-interval).   | Duration       | 5s             |
...
### Початковий інтервал повторного підключення {#reconnect-initial-interval}

Коли OBI втрачає зʼєднання з API Kubernetes, це значення контролює початкову затримку перед повторною спробою підключення.

Збільште це значення, щоб зменшити навантаження на нестабільні або перевантажені сервери API. Зменшіть його, коли потрібно швидше відновлення після тимчасових збоїв API.
